...
{
       0,    46,    46,    60,    67,    76,    83,    93,    92,   140,
     139,   174,   184,   194,   204,   216,   223,   233,   279,   286,
     293,   302,   309,   317,   324,   334,   341,   350,   357,   365,
     372,   379,   386,   393,   400,   407,   414,   423,   430,   439,
     447,   456,   463,   472,   479,   488,   495,   504,   512,   521,
     529,   539,   546,   553,   562,   569,   576,   583,   590,   597,
     604,   613,   621,   629,   636
};
#endif
...
#line 93 "syntax_analyzer.y"
    {
        ...
        string func_name = (yyvsp[(2) - (5)])->getname();
        string ret_type = (yyvsp[(1) - (5)])->getname();
...
#line 124 "syntax_analyzer.y"
    {
        ...
        string code = (yyvsp[(1) - (7)])->getname() + " " + (yyvsp[(2) - (7)])->getname() + "(" + (yyvsp[(4) - (7)])->getname() + ")\n" + (yyvsp[(7) - (7)])->getname();
        ...
        (yyval) = new symbol_info(code, "func_def");
    }
    break;
...
#line 140 "syntax_analyzer.y"
    {
        string func_name = (yyvsp[(2) - (4)])->getname();
        string ret_type = (yyvsp[(1) - (4)])->getname();
        ...
        if (sym_table->lookup_current_scope(func_name)) {
            outlog << "Error at line " << lines 
                   << ": Multiple declaration of function " << func_name << endl << endl;
            delete func_sym;
        } else {
            sym_table->insert(func_sym);
        }
...
#line 157 "syntax_analyzer.y"
    {
        ...
        string code = (yyvsp[(1) - (6)])->getname() + " " + (yyvsp[(2) - (6)])->getname() + "()\n" + (yyvsp[(6) - (6)])->getname();
        outlog << code << endl << endl;
...
#line 234 "syntax_analyzer.y"
    {
        ...
        outlog << "At line no: " << lines 
               ...
               << endl << endl;
        outlog << current_var_type << " " << (yyvsp[(2) - (3)])->getname() << ";" << endl << endl;

        // ---- INSERT ALL VARIABLES INTO SYMBOL TABLE ----
        string decl = (yyvsp[(2) - (3)])->getname();
        ...
        while (getline(ss, token, ',')) {
            ...
            if (pos != string::npos) {
                ...
                string sz_str = token.substr(pos+1, token.find(']') - pos - 1);
                size = sz_str.empty() ? 0 : stoi(sz_str);
            } else {
                name = token;
            }
            // trim whitespace
            ...
            if (sym_table->lookup_current_scope(name)) {
                outlog << "Error at line " << lines 
                       << ": Multiple declaration of " << name << endl << endl;
            } else {
                symbol_info* sym;
                if (size != -1) {
                    sym = new symbol_info(name, "ID", "ARRAY", current_var_type, size);
                } else {
                    sym = new symbol_info(name, "ID", "VARIABLE", current_var_type);
                }
                sym_table->insert(sym);
            }
        }

        (yyval) = new symbol_info(current_var_type + " " + (yyvsp[(2) - (3)])->getname() + ";", "var_dec");
    }
    break;
...
#line 303 "syntax_analyzer.y"
    {
        outlog << "At line no: " << lines 
               << " declaration_list : declaration_list COMMA ID " << endl << endl;
        outlog << (yyvsp[(1) - (3)])->getname() << "," << (yyvsp[(3) - (3)])->getname() << endl << endl;
...
#line 310 "syntax_analyzer.y"
    {
        ...
        string arr = (yyvsp[(3) - (6)])->getname() + "[" + (yyvsp[(5) - (6)])->getname() + "]";
        outlog << (yyvsp[(1) - (6)])->getname() << "," << arr << endl << endl;
        (yyval) = new symbol_info((yyvsp[(1) - (6)])->getname() + "," + arr, "decl_list");
    }
    break;
...
#line 325 "syntax_analyzer.y"
    {
        outlog << "At line no: " << lines 
               << " declaration_list : ID LTHIRD CONST_INT RTHIRD " << endl << endl;
        string arr = (yyvsp[(1) - (4)])->getname() + "[" + (yyvsp[(3) - (4)])->getname() + "]";
        outlog << arr << endl << endl;
        (yyval) = new symbol_info(arr, "decl_list");
    }
    break;
